module Site4
  # Recibe el formulario público de Site4. Crea exactamente el mismo `Inquiry`
  ...
  class InquiriesController < BaseController
    rate_limit to: 8, within: 5.minutes, only: :create,
               with: -> { redirect_back fallback_location: site4_contact_path,
                                        alert: t("site4.inquiries.rate_limited"), status: :see_other }
    ...
    def create
      ...
      if result.ok?
        log_outcome(result)
        redirect_to success_path, notice: t("site4.inquiries.created"), status: :see_other
      else
    ...
    def success_path
      requested_vehicle ? site4_vehicle_path(requested_vehicle) : site4_contact_path
    end

    def render_invalid(inquiry)
      @inquiry = inquiry
      flash.now[:alert] = t("site4.inquiries.invalid")
      ...
      if (vehicle = requested_vehicle)
        ...
        render "site4/vehicles/show", status: :unprocessable_content
      else
        @vehicles = Vehicles::Public.call.includes(:brand, :vehicle_model)
                                    .order(:brand_id, :vehicle_model_id).limit(200)
        render "site4/pages/contact", status: :unprocessable_content
      end
    end
    ...
    def log_outcome(result)
      ...
      Rails.logger.info("[inquiry/v4] descartada por #{result.status} desde #{request.remote_ip}")
    end
  end
end
